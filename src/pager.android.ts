...
export class Pager extends PagerBase {
    ...
    onLoaded(): void {
        ...
        if (!this.items && this._childrenCount > 0) {
            ...
            if (!(this._pagerAdapter instanceof StaticPagerStateAdapter)) {
                this._pagerAdapter = new StaticPagerStateAdapter(
                    new WeakRef(this)
                );
                this.pager.setAdapter(this._pagerAdapter);
                selectedIndexProperty.coerce(this);
    ...
    static getProgress(indicator, position, positionOffset, isRtl) {
        ...
        let slideToRightSide =
            selectedPosition == position && positionOffset != 0;
...
function initPagerRecyclerAdapter() {
    ...
    @NativeClass
    class PagerRecyclerAdapterImpl extends androidx.recyclerview.widget
            ...
            if (owner) {
                ...
                if (holder.view[PLACEHOLDER]) {
                    if (args.view) {
                        ...
                        holder.view.addChild(
                            owner._getDefaultItemContent(index)
                        );
                    }
...
function initStaticPagerStateAdapter() {
    ...
    @NativeClass
    class StaticPagerStateAdapterImpl extends androidx.recyclerview.widget
        .RecyclerView.Adapter<any> {
        owner: WeakRef<Pager>;
        ...
        public getItemId(i: number) {
            ...
            if (owner) {
                ...
                if (item) {
                    id = owner.itemIdGenerator(
                        item,
                        i,
                        Array.from(owner._childrenViews)
                    );
                }
            }
            return long(id);
...
function initPagerViewHolder() {
    ...
    @NativeClass
    class PagerViewHolderImpl extends androidx.recyclerview.widget.RecyclerView
        .ViewHolder {
        constructor(
